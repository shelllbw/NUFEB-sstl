...
void FixKineticsDiffusion::init() {
  ...
  setup_exchange(kinetics->grid, kinetics->subgrid.get_box(), { xpbcflag == 0, ypbcflag == 0, zpbcflag == 0 });
}
...
void FixKineticsDiffusion::compute_bc(double &nuCell, double *nuPrev, int grid, double bulk) {
  ...
  if (xgrid[grid][2] < zlo && grid_type[up] == REGULAR) {
    //0=PERIODIC-PERIODIC,  1=DIRiCH-DIRICH, 2=NEU-DIRICH, 3=NEU-NEU, 4=DIRICH-NEU
    if (zpbcflag == PP && kinetics->nz == kinetics->subn[2]) {
  ...
  // low-y surface
    ...
    if (ypbcflag == PP && kinetics->ny == kinetics->subn[1]) {
  ...
  // low-x surface
    ...
    if (xpbcflag == PP && kinetics->nx == kinetics->subn[0]) {
      int xhiGrid = grid + nx;
...
void FixKineticsDiffusion::update_nus() {
  if ((xpbcflag == PP || xpbcflag == NN) && (ypbcflag == PP || ypbcflag == NN) && (zpbcflag == PP || zpbcflag == NN)) {
    double **nus = kinetics->nus;
    double **nur = kinetics->nur;
...
void FixKineticsDiffusion::migrate(const Grid<double, 3> &grid, const Box<int, 3> &from, const Box<int, 3> &to) {
  ...
  setup_exchange(grid, to, { xpbcflag == 0, ypbcflag == 0, zpbcflag == 0 });
