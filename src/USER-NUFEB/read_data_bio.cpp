...
ReadDataBIO::ReadDataBIO(LAMMPS *lmp) : Pointers(lmp)
{
...
void ReadDataBIO::command(int narg, char **arg)
{
...
void ReadDataBIO::header(int firstpass)
{
  int n;
  char *ptr;
...
void ReadDataBIO::bonds(int firstpass)
{
  int nchunk,eof;
...
void ReadDataBIO::angles(int firstpass)
{
  int nchunk,eof;
...
void ReadDataBIO::dihedrals(int firstpass)
{
  int nchunk,eof;
...
void ReadDataBIO::impropers(int firstpass)
{
  int nchunk,eof;
...
void ReadDataBIO::bonus(bigint nbonus, AtomVec *ptr, const char *type)
{
  int nchunk,eof;
...
void ReadDataBIO::bodies(int firstpass)
{
...
void ReadDataBIO::anglecoeffs(int which)
{
...
void ReadDataBIO::dihedralcoeffs(int which)
{
...
void ReadDataBIO::impropercoeffs(int which)
{
...
void ReadDataBIO::fix(int ifix, char *keyword)
{
  int nchunk,eof;
...
int ReadDataBIO::reallocate(int **pcount, int cmax, int amax)
{
  int *count = *pcount;
...
void ReadDataBIO::open(char *file)
{
  compressed = 0;
...
void ReadDataBIO::parse_keyword(int first)
{
  int eof = 0;
  int done = 0;
...
void ReadDataBIO::skip_lines(bigint n)
{
  if (me) return;
  if (n <= 0) return;
  char *eof = NULL;
...
void ReadDataBIO::parse_coeffs(char *line, const char *addstr,
...
void ReadDataBIO::nutrient_memory(){
  ...
  bio->init_nus = memory->create(bio->init_nus,nnus+1,7,"bio:nuConc");
...
void ReadDataBIO::nutrients()
{
  ...
  if (me == 0) {
    if (screen) fprintf(screen,"  %i nutrients\n",nnus);
    if (logfile) fprintf(logfile,"  %i nutrients\n",nnus);
  }

  int eof = comm->read_lines_from_file(fp,nnus,MAXLINE,buf);
  if (eof) error->all(FLERR,"Unexpected end of data file");
...
void ReadDataBIO::yield()
{
  ...
  bio->yield = memory->create(bio->yield,atom->ntypes+1,"bio:yield");
...
void ReadDataBIO::maintain()
{
  ...
  bio->maintain = memory->create(bio->maintain,atom->ntypes+1,"bio:maintain");

  char *original = buf;
  for (i = 0; i < atom->ntypes; i++) {
    next = strchr(buf,'\n');
    *next = '\0';
    bio->set_maintain(buf);
    buf = next + 1;
  }
  delete [] original;
}
...
void ReadDataBIO::decay()
{
  ...
  bio->decay = memory->create(bio->decay,atom->ntypes+1,"bio:decay");
...
void ReadDataBIO::dissipation()
{
  ...
  bio->dissipation = memory->create(bio->dissipation,atom->ntypes+1,"bio:dissipation");

  char *original = buf;
  for (i = 0; i < atom->ntypes; i++) {
    next = strchr(buf,'\n');
    *next = '\0';
    bio->set_dissipation(buf);
    buf = next + 1;
  }
  delete [] original;
}
