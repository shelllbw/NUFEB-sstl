package nufeb.examples.ibm.detach;
...
public class IBMDetach {
	...
	public static void main(String[] args) throws Exception {
		...
		check(script, graph, getDetachParams());
	...
	private static ArrayList<HashMap<String, Double>> getDetachParams() {
		ArrayList<HashMap<String, Double>> toReturn = new ArrayList<>();
		double pre = 0;
		for (double i = 0; i < 594000; i = i + 1800) {
			// define sstl region
			HashMap<String,Double> map1 = new HashMap<>();
			map1.put("minT", pre);
			map1.put("maxT", i);
			map1.put("fromX", 1.8e-4);
			map1.put("toX", 1.834e-4);
			toReturn.add(map1);
	...
	public static void check(jSSTLScript script, GraphModel graph, ArrayList<HashMap<String,Double>> params) throws IOException {
		...
		Signal s = readMeshTraj(graph, "models/trajectories/shear-0.3/volf_all.txt");
		Signal s1 = readMeshTraj(graph, "models/trajectories/shear-0.3/gridx.txt");
		...
		for (int i = 0; i < params.size(); i++) {
			...
			PrintWriter stprinter = new PrintWriter("results/detach-0.3/"+"t" +i+".txt");
			...
			System.out.print("checking : " + parValues.get("maxT") + "-x" + + parValues.get("fromX"));
			
			for (int j = 0; j < graph.getNumberOfLocations(); j++) {
				if(boolSat2[j] > 0 && boolSat3[j] > 0) {
